...
<div class="card" style="margin-top:9rem">
    ...
    <div class="card-body">
     @if (request()->id == 0)
      ...
      @csrf
    @else
    <form method="POST" action="{{ url('plantinfo_update',['id'=>$plant_info->id])}}">
      @csrf
      @method('PUT')
    @endif
        ...
        <div class="row">
          <div class="col-md-3 p1-5">
            <div class="form-group">
                ...
                <input class="form-control" type="text" name="plant_name" placeholder="Enter plant name" required value="{{ is_null($plant_info->plant_name) ? '' : $plant_info->plant_name }}">
            </div>
          </div>
          <div class="col-md-3 p1-5">
            <div class="form-group">
                <label for="">Plant Life Duration</label>
                <input class="form-control" type="number" name="plant_life_duration" placeholder="Enter plant life duration" required value="{{ is_null($plant_info->plant_life_duration) ? '' : $plant_info->plant_life_duration }}">
            </div>
          </div>
          </div>
          <div class="row">
            <div class="col-md-6 p1-5">
              <div class="form-group">
                ...
                <textarea name="plant_description" class="form-control" id="" cols="30" rows="10" required>{{ is_null($plant_info->plant_description) ? '' : $plant_info->plant_description }}</textarea>
              </div>
            </div>
          </div>
